...
export class BBCGSAASDevice
	extends EventEmitter<DeviceEvents>
	implements Device<BBCGSAASOptions, BBCGSAASDeviceState, BBCGSAASDeviceCommand>
{
	...
	convertTimelineStateToDeviceState(state: Timeline.TimelineState<TSRTimelineContent>, mappings: Mappings): BBCGSAASDeviceState {
		...
		for (const layer of Object.keys(state.layers)) {
			...
			if (content.deviceType === DeviceType.BBC_GSAAS && mapping) {
				switch (content.type) {
					case TimelineContentTypeBBCGSAAS.CLEAR:
					...
					case TimelineContentTypeBBCGSAAS.UPDATE:
						...
						const { zone } = mapping.options
						newState[group][channel].data[zone] = {
							tlObjId: id,
							take: content.take,
							clear: content.clear
						}
	...
	diffStates(oldState: BBCGSAASDeviceState | undefined, newState: BBCGSAASDeviceState): Array<BBCGSAASDeviceCommand> {
		const commands: Array<BBCGSAASDeviceCommand> = []
				...
				for (const channelId of Object.keys(newState[groupId])) {
					...
					if (oldState[groupId][channelId]) {
						...
						if (Object.keys(newChannel.scenes).length === 0) {
							commands.push({
								tlObjId: newChannel.scenes.tlObjId ?? '',
								context: `Cleared scenes for channel ${channelId} in group ${groupId}`,
								command: {
				...
				if (newState[groupId]) {
					for (const channelId of Object.keys(oldState[groupId])) {
						...
						if (newState[groupId][channelId]) {
							...
							commands.push({
								tlObjId: oldChannel.scenes.tlObjId ?? '',
	...
	async sendCommand({ tlObjId, context, command }: BBCGSAASDeviceCommand): Promise<void> {
		...
		switch (type) {
			...
			default:
				this.emit(
					'warning',
					`BBC GSAAS - Invalid command type ${command.type} (${context})`,
				)
				return Promise.resolve()
		}
		...
		try {
			...
			this.emit(
				'error',
				`BBC GSAAS response error on ${type} "${endpoint}" (${context})`,
				err
			)
