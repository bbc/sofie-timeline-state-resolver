...
import type { CommandWithContext, DeviceTimelineState, DeviceTimelineStateObject } from 'timeline-state-resolver-api'
				...
				.then(async () => {
					...
					for (let i = 0; i < channelLength; i++) {
						...
						if (error) {
							...
							channelPromises.push(Promise.reject(new Error('execute failed')))
							continue
						}
